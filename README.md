...
这个目录会持续收纳可独立使用的 skill。当前仓库不是单一 skill 的根目录，而是多个 skill 的聚合目录，后续继续在这里新增别的 skill。
...
以 `project-agents-learning` 为例，必须手动触发，实际执行时由 Agent 按该目录下的 `SKILL.md` 定义流程运行 `scripts/project_agents_learning.py`，并处理目标项目里的 `AGENTS.md`。

## `project-agents-learning` 输出
